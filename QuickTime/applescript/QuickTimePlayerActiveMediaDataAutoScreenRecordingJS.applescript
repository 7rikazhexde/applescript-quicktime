/********************************************************
 ...
 Usage:
	1.QuickTime Player settings
		Press command+shift+5 to activate the screenshot toolbar.
...
function main(){
	var app = Application.currentApplication();
	app.includeStandardAdditions = true;
	if (quicktimeplayer_runnning_check()){
		var input_num = ask_recording_time()
		if(input_num > 0){
			if(quicktimeplayer_play_multiscreens()){
				screen_recording(input_num);
				quit_quicktimeplayer();
				app.displayAlert('Screen recording is complete.€nQuit script.');
			}
			else{
				quit_quicktimeplayer();
				app.displayAlert('Video file does not exist.€nQuit script.');
			}
		}
		else{
			//quit_quicktimeplayer();
		...
		app.displayAlert('Please Activate QuickTime Player.€nQuit script.');
	}
